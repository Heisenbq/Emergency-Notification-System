...
//    private ContactController contactController;
    ...
    void getContact_Test() throws Exception {
        ...
        mockMvc.perform(get("/contacts/2")
                        ...
                        .content(objectMapper.writeValueAsString(contact1)))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.contactName").value("Jah"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.phone").value("8-111-535-35-35"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("1234@mail.ru"));
    }
    ...
    @Test
    void updateContact_requestIsValid_returnUpdatedContactInResponseEntity() throws Exception {
        Contact contact = new Contact();
        contact.setContactName("John Doe");
        contact.setStatus("active");
        contact.setPhone("8-800-535-35-35");
        ...
        Mockito.when(contactService.updateContact((long) 1,contact)).thenReturn(contact);
        mockMvc.perform(post("/contacts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .characterEncoding("UTF-8")
                        .content(objectMapper.writeValueAsString(contact))).andDo(print())
                .andExpect(MockMvcResultMatchers.status().isOk());
    }
